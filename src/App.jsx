...
function ReadinessScore({ docs, completedDocs }) {
  const pct = docs.length ? Math.round((completedDocs.length / docs.length) * 100) : 0;
    ...
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 16, marginBottom: 18 }}>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 120 }}>
          ...
          <div style={{ marginTop: 12, height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 4 }}>
          ...
          {tab === "info" ? (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                {[{ label: "DEADLINE", value: new Date(s.deadline).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) }, { label: "COMPETITION", value: s.difficulty }].map(item => (
                  <div key={item.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: "11px 13px" }}>
...
function TrackerView({ scholarships, trackingData, onOpen }) {
  ...
  if (!tracked.length) return (
    <div style={{ textAlign: "center", padding: "70px 20px" }}>
      ...
      <div style={{ color: "#334155", fontSize: 13 }}>Browse → open a scholarship → click "Track This".</div>
    </div>
  );
  return (
    <div>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#07111d", fontFamily: "'Outfit',sans-serif", color: "#f1f5f9" }}>
      ...
      <DetailModal s={selected} tracking={selected ? trackingData[selected.id] : null} onClose={() => setSelected(null)} onUpdateTracking={(id, data) => { updateTracking(id, data); setSelected(null); }} />
    </div>
  );
}
